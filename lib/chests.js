...
const MATERIALS = [
  { item: 'ship parts', weight: 30 },
  { item: 'wood planks', weight: 30 },
  { item: 'iron ore', weight: 10 },
  { item: 'refined steel', weight: 10 },
  { item: 'weapon blueprint', weight: 10 },
  { item: 'armor fragment', weight: 10 }
];
...
export function getChestRewards(rank, inventoryItems) {
  ...
  if (r === 'C') {
    ...
    if (Math.random() < 0.50) rewards.xpBooks += 1;
